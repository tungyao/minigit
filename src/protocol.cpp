...
bool ProtocolMessage::deserialize(const vector<uint8_t> &data, ProtocolMessage &msg) {
	...
	if (msg.header.payload_size > 0) {
		msg.payload.insert(msg.payload.end(), data.begin() + sizeof(MessageHeader),
...
// 创建认证响应消息
ProtocolMessage ProtocolMessage::createAuthResponse(StatusCode status, const string &session_id,
													uint32_t timeout) {
	...
	memcpy(auth_response.session_id, hash.c_str(),
		   min(hash.size(), sizeof(auth_response.session_id)));
...
// 创建仓库列表响应
ProtocolMessage
ProtocolMessage::createRepoListResponse(const vector<pair<string, RepoListItem>> &repos) {
	vector<uint8_t> data;
...
// 创建克隆数据开始消息
ProtocolMessage ProtocolMessage::createCloneDataStart(const string &repo_name, uint32_t total_files,
													  uint64_t total_size) {
...
// 创建克隆文件消息
ProtocolMessage ProtocolMessage::createCloneFile(const string &file_path,
...
// 创建推送检查请求消息
ProtocolMessage ProtocolMessage::createPushCheckRequest(const string &local_head,
														const string &new_commit_id,
														const string &commit_parent) {
	...
	vector<uint8_t> data;
	data.resize(sizeof(PushCheckRequestPayload) + local_head.size() + new_commit_id.size() +
				commit_parent.size());
...
// 创建推送检查响应消息
ProtocolMessage ProtocolMessage::createPushCheckResponse(const string &remote_head,
														 bool needs_update) {
...
// 创建推送提交数据消息
ProtocolMessage ProtocolMessage::createPushCommitData(const string &commit_id,
													  const vector<uint8_t> &commit_data) {
...
// 创建推送对象数据消息
ProtocolMessage ProtocolMessage::createPushObjectData(const string &object_id,
													  const vector<uint8_t> &object_data) {
...
// 创建推送压缩对象数据消息
ProtocolMessage
ProtocolMessage::createPushObjectDataCompressed(MessageType type, uint32_t operation_type,
	...
	// 复制压缩数据
	memcpy(data.data() + sizeof(PushObjectDataPayloadCompressed), compressed_data.data(),
...
// 创建拉取检查响应消息
ProtocolMessage ProtocolMessage::createPullCheckResponse(const string &remote_head,
														 bool has_updates, uint32_t commits_count) {
...
// 创建拉取提交数据消息
ProtocolMessage ProtocolMessage::createPullCommitData(const string &commit_id,
													  const vector<uint8_t> &commit_data) {
...
// 创建拉取对象数据消息
ProtocolMessage ProtocolMessage::createPullObjectData(const string &object_id,
													  const vector<uint8_t> &object_data) {
...
// 创建拉取压缩对象数据消息
ProtocolMessage
ProtocolMessage::createPullObjectDataCompressed(uint32_t operation_type,
												const vector<uint8_t> &compressed_data,
												uint64_t original_size, uint32_t file_count) {
	...
	// 复制压缩数据
	memcpy(data.data() + sizeof(PullObjectDataPayloadCompressed), compressed_data.data(),
		   compressed_data.size());
...
// 创建克隆压缩数据消息
ProtocolMessage ProtocolMessage::createCloneDataCompressed(uint32_t operation_type,
														   const vector<uint8_t> &compressed_data,
	...
	// 复制压缩数据
	memcpy(data.data() + sizeof(CloneDataCompressedPayload), compressed_data.data(),
		   compressed_data.size());
...
ProtocolMessage ProtocolMessage::createLogResponse(const vector<pair<string, string>> &commits) {
	...
	for (const auto &commit : commits) {
		...
		memcpy(data.data() + old_size + sizeof(uint32_t), commit.first.c_str(),
			   commit.first.size());
		...
		memcpy(data.data() + old_size + sizeof(uint32_t), commit.second.c_str(),
			   commit.second.size());
	}
