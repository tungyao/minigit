...
// 兼容性别名（保持向后兼容）
using SHA256 = SHA1;
inline string sha256_string(const string& str) { return sha1_string(str); }
inline string sha256_bytes(const vector<unsigned char>& data) { return sha1_bytes(data); }
inline string sha256_file(const fs::path& p) { return sha1_file(p); }