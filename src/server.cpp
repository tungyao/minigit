...
bool Server::handleAuthRequest(int client_socket, shared_ptr<ClientSession> session, const ProtocolMessage &msg) {
    ...
    if (auth_payload.auth_type == 0) {
        ...
        if (!Config::getInstance().password.empty()) {
            ...
            cout << "Debug: Lengths - server: " << Config::getInstance().password.length() << ", provided: " <<
                    provided_password.length() << endl;
#endif // DEBUG
...
bool Server::handlePushCommitData(int client_socket, shared_ptr<ClientSession> session, const ProtocolMessage &msg) {
    ...
    if (msg.payload.size() < sizeof(PushCommitDataPayload) + commit_payload.commit_id_length + commit_payload.
        commit_data_length) {
...
bool Server::handlePushObjectDataCompressed(int client_socket, shared_ptr<ClientSession> session,
    ...
    if (msg.payload.size() < sizeof(PushObjectDataPayloadCompressed) + object_payload.compressed_size) {
    ...
    vector<uint8_t> object_data(msg.payload.begin() + sizeof(PushObjectDataPayloadCompressed),
                                msg.payload.begin() + sizeof(PushObjectDataPayloadCompressed) + object_payload.
                                compressed_size);
    ...
    fs::create_directories(local_repo_path);
    	bool extraction_success  = CompressionUtils::extractCompressedArchive(object_data, local_repo_path,
...
bool Server::handlePushObjectData(int client_socket, shared_ptr<ClientSession> session, const ProtocolMessage &msg) {
    ...
    if (msg.payload.size() < sizeof(PushObjectDataPayload) + object_payload.object_id_length + object_payload.
        object_data_length) {
        sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST, "Incomplete object data");
        return false;
    }
