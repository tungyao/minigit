...
bool Server::validatePushCommitIsLatest(const string &repo_name, const string &client_commit_parent,
...
// 处理协议消息
bool Server::processMessage(int client_socket, shared_ptr<ClientSession> session,
	...
	case MessageType::PUSH_OBJECT_DATA_COMPRESSED:
		return handlePushObjectDataCompressed(client_socket, session, msg);
...
// 发送克隆文件
bool Server::sendCloneFile(int client_socket, const string &relative_path,
	...
	try {
		...
		// 创建并发送文件消息
		auto file_msg =
			ProtocolMessage::createCloneFile(relative_path, file_data, 0); // 0 = 文件类型
...
// 认证请求处理
bool Server::handleAuthRequest(int client_socket, shared_ptr<ClientSession> session,
							   const ProtocolMessage &msg) {
	...
	vector<uint8_t> auth_data(msg.payload.begin() + sizeof(AuthRequestPayload),
							  msg.payload.begin() + sizeof(AuthRequestPayload) +
								  auth_payload.data_size);
	...
	if (auth_payload.auth_type == 0) {
		...
		if (!Config::getInstance().password.empty()) {
			...
			cout << "Debug: Lengths - server: " << Config::getInstance().password.length()
				 << ", provided: " << provided_password.length() << endl;
#endif // DEBUG
...
// 登录请求处理
bool Server::handleLoginRequest(int client_socket, shared_ptr<ClientSession> session,
								const ProtocolMessage &msg) {
	...
	auto response =
		ProtocolMessage::createStringMessage(MessageType::LOGIN_RESPONSE, "Login successful");
	return NetworkUtils::sendMessage(client_socket, response);
}

// 列出仓库请求处理
bool Server::handleListReposRequest(int client_socket, shared_ptr<ClientSession> session,
									const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
	...
	for (const string &repo_name : repos) {
		...
		item.last_modified =
			static_cast<uint64_t>(chrono::system_clock::now().time_since_epoch().count());
...
// 使用仓库请求处理
bool Server::handleUseRepoRequest(int client_socket, shared_ptr<ClientSession> session,
								  const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
	...
	if (!impl_->repo_manager->repositoryExists(repo_name)) {
		...
		if (!impl_->repo_manager->createRepository(repo_name)) {
			sendErrorResponse(client_socket, StatusCode::REPO_NOT_FOUND,
...
// 创建仓库请求处理
bool Server::handleCreateRepoRequest(int client_socket, shared_ptr<ClientSession> session,
									 const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
...
// 删除仓库请求处理
bool Server::handleRemoveRepoRequest(int client_socket, shared_ptr<ClientSession> session,
									 const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
...
bool Server::handlePushRequest(int client_socket, shared_ptr<ClientSession> session,
	...
	if (fs::exists(remote_head_path)) {
		try {
			...
			if (head_file.is_open()) {
				...
				head_file.close();
				// 移除尾部的换行符
				while (!current_remote_head.empty() &&
					   (current_remote_head.back() == '\n' || current_remote_head.back() == '\r')) {
	...
	if (request_payload.remote_head_length > 0) {
		if (msg.payload.size() < sizeof(PushRequestPayload) + request_payload.remote_head_length) {
			sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST,
							  "Invalid local head data");
			return false;
		}
		new_remote_head =
			string(reinterpret_cast<const char *>(msg.payload.data() + sizeof(PushRequestPayload)),
	...
	if (!new_remote_head.empty()) {
		...
		cout << "commit_obj_path " << commit_obj_path << endl;
		if (fs::exists(commit_obj_path)) {
			try {
				...
				if (commit_file.is_open()) {
					...
					Commit commit =
						CommitManager::deserializeCommit(new_remote_head + "\n" + commit_content);
			...
			sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST,
							  "Commit object not found, please upload commit data first");
			return false;
		}

		// 验证提交是否基于最新版本
		if (!validatePushCommitIsLatest(session->current_repo, client_commit_parent,
										current_remote_head)) {
			sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST,
							  "Push rejected: commit is not based on the latest version. Please "
	...
	if (!new_remote_head.empty()) {
		try {
			...
			sendErrorResponse(client_socket, StatusCode::SERVER_ERROR,
							  "Failed to update remote HEAD");
			return false;
		}
	}

	auto response =
		ProtocolMessage::createStringMessage(MessageType::PUSH_RESPONSE, "Push completed");
	return NetworkUtils::sendMessage(client_socket, response);
}
...
bool Server::handlePushCheckRequest(int client_socket, shared_ptr<ClientSession> session,
	...
	// 检查数据完整性
	size_t required_size = sizeof(PushCheckRequestPayload) + check_payload.local_head_length +
						   check_payload.new_commit_id_length + check_payload.commit_parent_length;
	if (msg.payload.size() < required_size) {
		sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST,
						  "Incomplete push check request data");
		return false;
	}
	...
	if (check_payload.local_head_length > 0) {
		local_head = string(reinterpret_cast<const char *>(msg.payload.data() + offset),
	...
	if (check_payload.new_commit_id_length > 0) {
		new_commit_id = string(reinterpret_cast<const char *>(msg.payload.data() + offset),
	...
	if (check_payload.commit_parent_length > 0) {
		commit_parent = string(reinterpret_cast<const char *>(msg.payload.data() + offset),
	...
	if (!fs::exists(remote_head_path)) {
		// 检查如果不存在 则新建
		if (impl_->repo_manager->createRepository(session->current_repo)) {
			cout << "create new repo success " << session->current_repo << endl;
		} else {
			cout << "create new repo failed " << session->current_repo << endl;
		}
	}
	if (fs::exists(remote_head_path)) {
		try {
...
// 推送提交数据处理
bool Server::handlePushCommitData(int client_socket, shared_ptr<ClientSession> session,
								  const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
	...
	if (msg.payload.size() < sizeof(PushCommitDataPayload) + commit_payload.commit_id_length +
								 commit_payload.commit_data_length) {
	...
	// 提取commit ID
	string commit_id(
		reinterpret_cast<const char *>(msg.payload.data() + sizeof(PushCommitDataPayload)),
		commit_payload.commit_id_length);

	// 提取commit数据
	vector<uint8_t> commit_data(
		msg.payload.begin() + sizeof(PushCommitDataPayload) + commit_payload.commit_id_length,
		msg.payload.begin() + sizeof(PushCommitDataPayload) + commit_payload.commit_id_length +
			commit_payload.commit_data_length);
...
// 推送压缩对象数据处理  包含多个commit的全部对象数据
bool Server::handlePushObjectDataCompressed(int client_socket, shared_ptr<ClientSession> session,
											const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
	...
	// 解析对象数据
	if (msg.payload.size() < sizeof(PushObjectDataPayloadCompressed)) {
		sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST, "Invalid object data");
		return false;
	}
	PushObjectDataPayloadCompressed object_payload;
	memcpy(&object_payload, msg.payload.data(), sizeof(PushObjectDataPayloadCompressed));
	if (msg.payload.size() <
		sizeof(PushObjectDataPayloadCompressed) + object_payload.compressed_size) {
		sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST, "Incomplete object data");
		return false;
	}
	// 提取对象数据
	vector<uint8_t> object_data(msg.payload.begin() + sizeof(PushObjectDataPayloadCompressed),
								msg.payload.begin() + sizeof(PushObjectDataPayloadCompressed) +
									object_payload.compressed_size);

	// 验证数据校验和
	uint32_t calculated_checksum = ProtocolMessage::calculateCRC32(object_data);
	if (calculated_checksum != object_payload.checksum) {
		sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST,
						  "Object data checksum mismatch");
		return false;
	}
	fs::path local_repo_path = fs::current_path() / session->current_repo;
	fs::create_directories(local_repo_path);
	bool extraction_success = CompressionUtils::extractCompressedArchive(
		object_data, local_repo_path, [](int progress, const string &description) {
...
bool Server::handlePushObjectData(int client_socket, shared_ptr<ClientSession> session,
								  const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
	...
	if (msg.payload.size() < sizeof(PushObjectDataPayload) + object_payload.object_id_length +
								 object_payload.object_data_length) {
		sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST, "Incomplete object data");
		return false;
	}
	...
	// 提取对象ID
	string object_id(
		reinterpret_cast<const char *>(msg.payload.data() + sizeof(PushObjectDataPayload)),
		object_payload.object_id_length);

	// 提取对象数据
	vector<uint8_t> object_data(
		msg.payload.begin() + sizeof(PushObjectDataPayload) + object_payload.object_id_length,
		msg.payload.begin() + sizeof(PushObjectDataPayload) + object_payload.object_id_length +
			object_payload.object_data_length);
...
bool Server::handlePullRequest(int client_socket, shared_ptr<ClientSession> session,
							   const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
...
// 拉取检查请求处理
bool Server::handlePullCheckRequest(int client_socket, shared_ptr<ClientSession> session,
									const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
	...
	if (check_payload.local_head_length > 0) {
		if (msg.payload.size() <
			sizeof(PullCheckRequestPayload) + check_payload.local_head_length) {
			sendErrorResponse(client_socket, StatusCode::INVALID_REQUEST,
							  "Invalid local head data");
			return false;
		}
...
bool Server::handleCloneRequest(int client_socket, shared_ptr<ClientSession> session,
	...
	try {
		...
		// 发送克隆开始消息
		auto start_msg = ProtocolMessage::createCloneDataStart(
			repo_name, static_cast<uint32_t>(files_to_clone.size()), total_size);
		...
		uint32_t raw_size;
		bool compression_success = CompressionUtils::createCompressedArchive(
			relative_paths, repo_path, compressed_archive, raw_size,
			[](int progress, const string &description) {
				// 服务器端可以记录日志，但不显示进度
			});
		...
		if (compression_success) {
			...
			for (const auto &file_info : files_to_clone) {
				if (!sendCloneFile(client_socket, file_info.first, file_info.second)) {
					sendErrorResponse(client_socket, StatusCode::SERVER_ERROR,
									  "Failed to send file: " + file_info.first);
					return false;
				}
			}
		}
...
// 处理日志请求
bool Server::handleLogRequest(int client_socket, shared_ptr<ClientSession> session,
							  const ProtocolMessage &msg) {
	if (!session->authenticated) {
		sendErrorResponse(client_socket, StatusCode::AUTH_REQUIRED, "Authentication required");
		return false;
...
void ServerCommand::printUsage() {
	cout << "Usage: minigit server --port <port> --root <path> [--password <password>] [--cert "
			"<cert_path>]\n";
	cout << "Options:\n";
