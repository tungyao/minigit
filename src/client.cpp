            ...
            for (uint32_t i = 0; i < repo_count && offset < response.payload.size(); ++i) {
                if (offset + sizeof(RepoListItem) <= response.payload.size()) {
                    ...
                    if (offset + item.name_length <= response.payload.size()) {
                        string repo_name(reinterpret_cast<const char *>(response.payload.data() + offset),
                                         item.name_length);
            ...
            for (uint32_t i = 0; i < log_payload.commits_count && offset < response.payload.size(); ++i) {
                ...
                if (offset + sizeof(uint32_t) <= response.payload.size()) {
                    ...
                    if (offset + commit_id_length <= response.payload.size()) {
                        string commit_id(reinterpret_cast<const char *>(response.payload.data() + offset),
                                         commit_id_length);
                        ...
                        if (offset + sizeof(uint32_t) <= response.payload.size()) {
                            ...
                            if (offset + message_length <= response.payload.size()) {
                                string message(reinterpret_cast<const char *>(response.payload.data() + offset),
                                               message_length);
...
bool Client::push() {
    ...
    if (check_payload.remote_head_length > 0) {
        ...
        remote_head = string(
            reinterpret_cast<const char *>(check_response.payload.data() + sizeof(PushCheckResponsePayload)),
    ...
    for (const string &commit_id: commits_to_upload) {
		...
		// 将commit文件也加入上传列表
        fs::path commit_path = FileSystemUtils::repoRoot() / ".minigit" / "objects" / commit_id;
    ...
    auto commit_push = ProtocolMessage::createPushObjectDataCompressed(MessageType::PUSH_OBJECT_DATA_COMPRESSED,
        0, compressed_archive, raw_size, files_to_push.size()
    );
...
bool Client::pull() {
    ...
    if (check_payload.remote_head_length > 0) {
        ...
        remote_head = string(
            reinterpret_cast<const char *>(check_response.payload.data() + sizeof(PullCheckResponsePayload)),
            check_payload.remote_head_length);
    }
...
void Client::setRemoteConfigForClone(const fs::path &local_repo_path, const string &repo_name) {
    try {
        ...
        string remote_url = "server://" + Config::getInstance().server_host + ":" + to_string(
                                Config::getInstance().server_port) + "/" + repo_name;
