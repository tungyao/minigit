		...
		for (int retry = 0; retry < max_retries; ++retry) {                                        \
			if (ensureConnected()) {                                                               \
				...
				if (retry < max_retries - 1) {                                                     \
					cout << "Network operation failed, retrying (" << (retry + 1) << "/"           \
						 << max_retries << ")...\n";                                               \
					connected_ = false;                                                            \
...
bool Client::connect() {
	...
	connected_ = true;
	cout << "Connected to " << Config::getInstance().server_host << ":"
		 << Config::getInstance().server_port << "\n";
...
bool Client::authenticate() {
	...
	if (!Config::getInstance().password.empty()) {
		...
		cout << "Debug: Client sending password: '" << Config::getInstance().password << "'"
			 << endl;
			...
			for (uint32_t i = 0; i < repo_count && offset < response.payload.size(); ++i) {
				if (offset + sizeof(RepoListItem) <= response.payload.size()) {
					...
					if (offset + item.name_length <= response.payload.size()) {
						string repo_name(
							reinterpret_cast<const char *>(response.payload.data() + offset),
							item.name_length);
...
vector<string> Client::log(int max_count, bool line) {
	...
	if (response.header.type == MessageType::LOG_RESPONSE) {
		...
		if (response.payload.size() >= sizeof(LogResponsePayload)) {
			...
			for (uint32_t i = 0; i < log_payload.commits_count && offset < response.payload.size();
				 ++i) {
				// 读取commit_id
				if (offset + sizeof(uint32_t) <= response.payload.size()) {
					...
					if (offset + commit_id_length <= response.payload.size()) {
						...
						if (offset + sizeof(uint32_t) <= response.payload.size()) {
							...
							memcpy(&message_length, response.payload.data() + offset,
								   sizeof(uint32_t));
							offset += sizeof(uint32_t);

							if (offset + message_length <= response.payload.size()) {
								string message(reinterpret_cast<const char *>(
...
bool Client::useRepository(const string &repo_name) {
	...
	if (response.header.type == MessageType::USE_REPO_RESPONSE) {
		...
		FileSystemUtils::getInstance().useRepo(repo_name);
...
bool Client::createRepository(const string &repo_name) {
	...
	auto request =
		ProtocolMessage::createStringMessage(MessageType::CREATE_REPO_REQUEST, repo_name);
	if (!NetworkUtils::sendMessage(client_socket_, request)) {
...
bool Client::removeRepository(const string &repo_name) {
	...
	auto request =
		ProtocolMessage::createStringMessage(MessageType::REMOVE_REPO_REQUEST, repo_name);
	if (!NetworkUtils::sendMessage(client_socket_, request)) {
...
bool Client::push() {
	...
	auto check_request =
		ProtocolMessage::createPushCheckRequest(local_head, new_commit_id, commit_parent);
	...
	if (check_payload.remote_head_length > 0) {
		if (check_response.payload.size() <
			sizeof(PushCheckResponsePayload) + check_payload.remote_head_length) {
			cerr << "Invalid remote head data\n";
			return false;
		}
		remote_head = string(reinterpret_cast<const char *>(check_response.payload.data() +
															sizeof(PushCheckResponsePayload)),
	...
	for (const string &commit_id : commits_to_upload) {
		...
		for (const auto &tree_item : commit_opt->tree) {
			files_to_push.push_back(tree_item.second);
			relative_paths.push_back(fs::path("objects") / tree_item.second);
		}
		// 将commit文件也加入上传列表
		fs::path commit_path =
			FileSystemUtils::getInstance().repoRoot() / ".minigit" / "objects" / commit_id;
		files_to_push.push_back(commit_path);
		relative_paths.push_back(fs::path("objects") / commit_id);
	}
	if (files_to_push.empty()) {
		cout << "No files to push\n";
		return true;
	}
	cout << "compress " << files_to_push.size() << " file(s)...\n";
	vector<uint8_t> compressed_archive;
	uint32_t raw_size;
	bool compression_success = CompressionUtils::createCompressedArchive(
		relative_paths, FileSystemUtils::getInstance().repoRoot() / ".minigit", compressed_archive,
		raw_size, [](int progress, const string &description) {
			ProgressDisplay::showCompressionProgress(progress, "compress", description);
		});
	...
	cout << "uploading commit(s) (" << ProgressDisplay::formatFileSize(compressed_archive.size())
		 << ")...\n";
...
bool Client::pull() {
	...
	cout << "Checking what to pull (local HEAD: "
		 << (local_head.empty() ? "(none)" : local_head.substr(0, 12)) << ")...\n";
	...
	if (check_payload.remote_head_length > 0) {
		if (check_response.payload.size() <
			sizeof(PullCheckResponsePayload) + check_payload.remote_head_length) {
			cerr << "Invalid remote head data\n";
			return false;
		}
		remote_head = string(reinterpret_cast<const char *>(check_response.payload.data() +
															sizeof(PullCheckResponsePayload)),
							 check_payload.remote_head_length);
	}
...
bool Client::clone(const string &repo_name) {
	...
	// 接收克隆数据
	if (!receiveCloneData(repo_name)) {
		cerr << "Failed to clone\n";
		return false;
	}
	fs::path head_path = fs::current_path() / repo_name / MARKNAME / "HEAD";
	string last_commit_id = FileSystemUtils::getInstance().readText(head_path);
	if (last_commit_id.empty()) {
		return true;
	}

	auto target = CommitManager::loadCommit(repo_name, last_commit_id);
	if (!target) {
		cerr << "Recover last commit: " << last_commit_id << "\n";
		return false;
	}
...
bool Client::createConnection() {
	...
	// 获取服务器地址信息
	status = getaddrinfo(Config::getInstance().server_host.c_str(),
						 std::to_string(Config::getInstance().server_port).c_str(), &hints, &res);
	if (status != 0) {
...
bool Client::performNetworkOperation(Operation operation, const string &operation_name,
									 int max_retries) {
	for (int attempt = 1; attempt <= max_retries; ++attempt) {
		...
		if (!NetworkUtils::isSocketConnected(client_socket_)) {
#ifdef DEBUG
			cout << operation_name << " failed due to connection issue, retrying (" << attempt
				 << "/" << max_retries << ")...\n";
#endif // DEBUG
			connected_ = false;
			authenticated_ = false;
...
bool Client::processCloneFile(const fs::path &local_repo_path, const ProtocolMessage &file_msg) {
	...
	string file_path(
		reinterpret_cast<const char *>(file_msg.payload.data() + sizeof(CloneFilePayload)),
...
// 处理克隆压缩数据
bool Client::processCloneCompressedData(const fs::path &local_repo_path,
	...
	bool extraction_success = CompressionUtils::extractCompressedArchive(
		compressed_data, local_repo_path, [](int progress, const string &description) {
...
void ClientCommand::printUsage() {
	cout << "Usage: minigit connect [--host <host>] [--port <port>] [--password <password>] "
			"[--cert <cert_path>]\n";
	cout << "Options:\n";
...
void Client::setRemoteConfigForClone(const fs::path &local_repo_path, const string &repo_name) {
	try {
		...
		string remote_url = "server://" + Config::getInstance().server_host + ":" +
							to_string(Config::getInstance().server_port) + "/" + repo_name;
...
vector<string> Client::getCommitsToUpload(const string &local_head, const string &remote_head) {
	...
	if (current_commit != remote_head) {
		cerr << "Warning: Remote commit not found in local history. This may indicate diverged "
				"branches.\n";
...
bool Client::uploadObject(const string &object_id) {
	fs::path object_path = FileSystemUtils::getInstance().objectsDir() / object_id;
...
bool Client::receiveCommitData(const ProtocolMessage &msg) {
	...
	if (msg.payload.size() <
		offset + commit_payload.commit_id_length + commit_payload.commit_data_length) {
		cerr << "Invalid commit data size\n";
		return false;
	}

	string commit_id = string(reinterpret_cast<const char *>(msg.payload.data() + offset),
							  commit_payload.commit_id_length);
	offset += commit_payload.commit_id_length;

	// 解析提交数据
	vector<uint8_t> commit_data(msg.payload.begin() + offset,
								msg.payload.begin() + offset + commit_payload.commit_data_length);

	cout << "Receiving commit " << commit_id.substr(0, 12) << "...\n";

	// 保存提交对象
	fs::path commit_path = FileSystemUtils::getInstance().objectsDir() / commit_id;
	if (!fs::exists(commit_path)) {
		...
		try {
			...
			commit_file.write(reinterpret_cast<const char *>(commit_data.data()),
							  commit_data.size());
			commit_file.close();
		} catch (const exception &e) {
...
bool Client::receiveObjectData(const ProtocolMessage &msg) {
	...
	if (msg.payload.size() <
		offset + object_payload.object_id_length + object_payload.object_data_length) {
		cerr << "Invalid object data size\n";
		return false;
	}

	string object_id = string(reinterpret_cast<const char *>(msg.payload.data() + offset),
							  object_payload.object_id_length);
	offset += object_payload.object_id_length;

	// 解析对象数据
	vector<uint8_t> object_data(msg.payload.begin() + offset,
								msg.payload.begin() + offset + object_payload.object_data_length);
	...
	if (!fs::exists(object_path)) {
		...
		try {
			...
			object_file.write(reinterpret_cast<const char *>(object_data.data()),
							  object_data.size());
			object_file.close();
		} catch (const exception &e) {
...
bool Client::receiveCompressedObjectData(const ProtocolMessage &msg) {
	...
	bool extraction_success = CompressionUtils::extractCompressedArchive(
		compressed_data, temp_extract_path, [](int progress, const string &description) {
			ProgressDisplay::showCompressionProgress(progress, "extract", description);
		});
	ProgressDisplay::finish();
	...
	if (fs::exists(extract_objects_path)) {
		...
		for (const auto &entry : fs::recursive_directory_iterator(extract_objects_path)) {
			if (entry.is_regular_file()) {
				...
				fs::path target_path = FileSystemUtils::getInstance().objectsDir() / relative_path;
