...
bool CompressionUtils::decompressData(const vector<uint8_t>& packed_data,
    ...
    int decompressed_size = LZ4_decompress_safe(
        reinterpret_cast<const char*>(packed_data.data() + 4),
        reinterpret_cast<char*>(output.data()),
        static_cast<int>(packed_data.size() - 4),
        original_size
    );
...
bool CompressionUtils::extractCompressedArchive(const vector<uint8_t> &archive_data,
    ...
    for (uint32_t i = 0; i < header.file_count; ++i) {
        ...
        // 写入文件
        fs::path full_output_path = output_path / MARKNAME / file_path_str;
